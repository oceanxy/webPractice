...
OpenLayers.Layer.GeoWebCache = OpenLayers.Class(OpenLayers.Layer.Grid, {
    ...
    getURL: function (bounds) {
        ...
        var ress = this.getServerResolution();
        var bbox = this.maxExtent;
        var size = this.tileSize;
        var tileX = Math.round((bounds.left - bbox.left) / (ress * size.w));
        var tileY = Math.round((bounds.bottom - bbox.bottom) / (ress * size.h));
        var tileZ = this.serverResolutions != null ? OpenLayers.Util.indexOf(this.serverResolutions, ress) : this.map.getZoom();

        // x is column index counting in the positive x direction from grid origin (lower left)
        var x = Math.floor((center.lon - this.gridBounds.left) / (res * this.tileSize.w));

        // y is row index counting in the positive y direction from the grid origin (lower left)
        var y = Math.floor((center.lat - this.gridBounds.bottom) / (res * this.tileSize.h));
        ...
        this.params.layers = "kmap:kmap";
        this.params.zoom = tileZ;
        this.params.x = tileX;
        this.params.y = tileY;
        if (this.params) {
        ...
        // base = base.replace(/\$\{x}/, xyz.x);
        // base = base.replace(/\$\{y}/, xyz.y);
        // base = base.replace(/\$\{z}/, xyz.z || z);
        // return base + "&zoom=" + xyz.z + "&x=" + xyz.x + "&y=" + xyz.y;
